...
<div class="cliente">
	...
	<div class="incluir">
		<fieldset>
			<legend id="legenda">Incluir Cliente</legend>
			<form id="formCadastroCliente">
				...
				<fieldset>
					...
					<div class="field" id="buttons">
						...
						<input type="button" id="cancel" onclick="cancelar('.incluir', '#formCadastroCliente')" value="Cancelar" name="submit">
					</div>
				</fieldset>
...
<script type="text/javascript">
	...
	function doSubmit(action) {
		...
		if (action == 'incluir') {
			...
			$.post("incluir.cliente.ajax.php", $("#formCadastroCliente").serialize())
				.success(function() {

					$('.msgbox').css('color', '#060');
					$('.msgbox').fadeIn('fast');
					$('.msgbox').html("Cliente inserido com sucesso!");
					$('.msgbox').fadeOut(5000);
					...
					$('#formCadastroCliente').get(0).reset();

				});

		} else if (action == 'alterar') {

			$.post("update.cliente.ajax.php", $("#formCadastroCliente").serialize())
				.success(function() {

					/* Transforma em função */
					$('.msgbox').css('color', '#060');
					$('.msgbox').fadeIn('fast');
					$('.msgbox').html("Cliente alterado com sucesso!");
					$('.msgbox').fadeOut(5000);
	...
	function cancelar(d, f) {
		...
		$('input#cancel').remove();
		$('div#buttons').append('<input type="button" id="cancel" onclick="cancelar(\'.incluir\', \'#formCadastroCliente\')" value="Cancelar" name="submit">');
	}
</script>
...
<script type="text/javascript">
	function doFlex() {
		...
		jQuery("#list1").flexigrid({
			url: 'pesquisa.cliente.ajax.php?uuid=<?php echo $uuid; ?>',
			dataType: "json",
			colModel: [{
					display: 'ID',
					name: 'cli_codigo',
	...
	function doCommand(com, grid) {
		if (com == 'Incluir') {
			...
			$('div#buttons').append('<input type="button" id="cancel" onclick="cancelar(\'.alterar\', \'#formCadastroCliente\')" value="Cancelar" name="submit">');
			...
			$('.trSelected', grid).each(function() {
				...
				if (confirm('Tem certeza que você deseja excluir o cliente selecionado?')) {

					$.post("delete.cliente.ajax.php", {
						cliente_id: id
					});

					$('.msgbox').css('color', '#060');
					$('.msgbox').fadeIn('fast');
					$('.msgbox').html("Cliente excluído com sucesso!");
					$('.msgbox').fadeOut(5000);
